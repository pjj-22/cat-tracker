...
try:
    ...
    print("[WARN] adafruit_servokit not available - running in camera-only mode")
...
class PanTiltController:
    ...
    def __init__(self, pan_channel=0, tilt_channel=1,
        ...
        if SERVO_AVAILABLE:
            ...
            self._kit.servo[pan_channel].angle = pan_center
            self._kit.servo[tilt_channel].angle = tilt_center
        else:
            self._kit = None
    ...
    def _set_pan(self, angle):
        self._pan = angle
        if self._kit:
            self._kit.servo[self.pan_channel].angle = angle

    def _set_tilt(self, angle):
        self._tilt = angle
        if self._kit:
            self._kit.servo[self.tilt_channel].angle = angle
